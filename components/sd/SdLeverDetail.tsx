...
import { SD_LEVER_DESCRIPTIONS } from "@/lib/sd/descriptions";
import { sdLeverProjection } from "./leverProjection";
import { signedMoneyM } from "./format";

/**
 * Click-to-open detail for a single lever: what it does, its scored effect now and the
 * projection to 2040, and the source behind the number.
 */
export function SdLeverDetail({ lever, cfg }: { lever: SdLever; cfg: SdLeverConfig }) {
  const desc = SD_LEVER_DESCRIPTIONS[lever.id];
  const proj = sdLeverProjection(lever, cfg);
    ...
    <Popover>
      ...
      <PopoverContent align="start" className="w-80 text-xs">
        ...
        <div className="mb-3 space-y-1 rounded-md border border-border/50 bg-card/40 p-2 font-mono">
          <Row label={`${verb}${enabledHint}, FY2027`} value={proj.y2027} />
          <Row label="Projected FY2040" value={proj.y2040} muted />
        </div>
...
function Row({ label, value, muted }: { label: string; value: number; muted?: boolean }) {
  const display = Math.abs(value) < 0.05 ? "no change at current setting" : `${signedMoneyM(value)}/yr`;
  return (
